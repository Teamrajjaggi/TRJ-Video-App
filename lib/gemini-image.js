...
async function callPredict({ model, prompt }) {
  const body = {
    ...
    parameters: {
      sampleCount: 1,
      aspectRatio: '9:16',
      // Imagen's defaults block real-person generation entirely. Loosen so
      // the playbook prompts can run; if a specific image gets filtered the
      // safety filter still catches it.
  ...
  if (!data) {
    // Surface why. Imagen will return raiFilteredReason / blocked when the
    // safety filter cut the result, or just empty {} for silent blocks.
    const filtered = pred?.raiFilteredReason || payload?.raiFilteredReason;
    if (filtered) {
      throw new Error(`Imagen blocked by safety filter (${filtered}): ${text.slice(0, 400)}`);
    }
    throw new Error(
      `Imagen returned no image data — likely silent safety block. Full response: ${text.slice(0, 600)}`,
    );
  }
  return {
...
async function callModel({ model, prompt, referenceUrls }) {
  if (isImagen(model)) return callPredict({ model, prompt });
  return callGenerateContent({ model, prompt, referenceUrls });
}
...
async function generateImage({ prompt, referenceUrls = [] }) {
  ...
  try {
    return await callModel({ model: PRIMARY_MODEL, prompt, referenceUrls });
  } catch (e) {
    if (!FALLBACK_MODEL || PRIMARY_MODEL === FALLBACK_MODEL) throw e;
    // Only fall back on transient/capacity issues. Non-transient (404, 401, 400)
    // means the request itself is wrong and the fallback won't fix that.
    ...
    return await callModel({ model: FALLBACK_MODEL, prompt, referenceUrls });
  }
}
