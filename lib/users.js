...
async function bootstrapAdmin() {
  ...
  if (existing) {
    // If ADMIN_PASSWORD is set in env, keep the admin's password in sync
    // with it on every boot. Lets the user set a memorable password once
    // without having to wipe data/users.json.
    if (process.env.ADMIN_PASSWORD) {
      const users = readUsers();
      const idx = users.findIndex((u) => u.id === existing.id);
      if (idx !== -1) {
        users[idx].passwordHash = await hashPassword(process.env.ADMIN_PASSWORD);
        writeUsers(users);
      }
