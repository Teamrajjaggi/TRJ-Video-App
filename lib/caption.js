...
const DEFAULT_BRAND_PROMPT =
  'You write short, punchy social-media captions for Team Raj Jaggi, ' +
  'posting AI-generated video clips to TikTok and Instagram Reels. ' +
  'The voice is energetic, modern, and confident — never corporate or ' +
  'hashtag-stuffed.';

// Every generated caption ends with this call-to-action. Override with the
// CAPTION_CTA env var (e.g. if the phone number changes).
const DEFAULT_CTA = '📞 Call Team Raj Jaggi at 516-996-3633';
...
async function generateCaption(video) {
  ...
  try {
    ...
    // Guarantee the call-to-action — Gemini writes only the caption body.
    const cta = process.env.CAPTION_CTA || DEFAULT_CTA;
    const caption = parsed.caption.includes(cta)
      ? parsed.caption
      : `${parsed.caption}\n\n${cta}`;
