...
// Auth schemes are configurable since Higgsfield gives both a Key ID
// and an API Key, and different endpoints sometimes want them combined.
// HIGGSFIELD_AUTH_MODE accepts: bearer | bearer-with-id | basic | x-api-key
function authHeaders() {
  const key = process.env.HIGGSFIELD_API_KEY;
  const keyId = process.env.HIGGSFIELD_KEY_ID || '';
  if (!key) throw new Error('HIGGSFIELD_API_KEY is not set');
  const mode = process.env.HIGGSFIELD_AUTH_MODE || 'bearer-with-id';
  const headers = {
  ...
  switch (mode) {
    ...
    default:
      headers.Authorization = `Bearer ${key}`;
      if (keyId) {
        headers['hf-api-key'] = key;
        headers['hf-key-id'] = keyId;
      }
      break;
  }
  return headers;
