...
const SUBJECTS_LOCAL_PATH = path.join(__dirname, '..', 'prompts', 'subjects.local.md');
...
// Parses prompts/subjects.local.md if present. Format:
//   # Subject A
//   ...description...
//   # Subject B
//   ...description...
function subjectDescriptions() {
  let body = '';
  try {
    body = fs.readFileSync(SUBJECTS_LOCAL_PATH, 'utf8');
  } catch {
    return {
      a: process.env.SUBJECT_A_DESCRIPTION || '',
      b: process.env.SUBJECT_B_DESCRIPTION || '',
    };
  }
  // Strip UTF-8/UTF-16 BOMs and any leading whitespace before the first heading.
  body = body.replace(/^[﻿​\s]+/, '');
  // Tolerate optional CR before \n. Match content up to the next "# Subject" heading or EOF.
  const matchA = body.match(/#\s*Subject\s*A\b[^\n]*\r?\n([\s\S]*?)(?=#\s*Subject\s*B\b|$)/i);
  const matchB = body.match(/#\s*Subject\s*B\b[^\n]*\r?\n([\s\S]*?)$/i);
  return {
    a: (matchA?.[1] || process.env.SUBJECT_A_DESCRIPTION || '').trim(),
    b: (matchB?.[1] || process.env.SUBJECT_B_DESCRIPTION || '').trim(),
  };
}
...
function composeImagePrompt(plan) {
  ...
  const { a: descA, b: descB } = subjectDescriptions();
  ...
  return interpolate(tmpl, {
    ...
    SUBJECT_A_DESCRIPTION: descA,
    SUBJECT_B_DESCRIPTION: descB,
