...
// Drive's pre-rendered thumbnail URL. The link points at Google's CDN so
// it's much faster than proxying the original bytes through this server.
// Returns null if Drive hasn't generated a thumbnail yet (freshly uploaded
// files take a few seconds). The thumbnailLink is cached per file id —
// regenerating it is a Drive API round trip on every request otherwise.
const thumbCache = new Map(); // drive_file_id -> { link, exp }
...
async function getThumbnailLink(fileId) {
  ...
  if (link) thumbCache.set(fileId, { link, exp: Date.now() + THUMB_CACHE_TTL_MS });
  return link;
}
...
// Permanently deletes a Drive file. Used by the trash purger to free
// storage once a rejected clip has aged out.
