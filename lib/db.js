...
function applyDeltaMigrations(d) {
  // approval_queue gained posted_at / posted_targets after the initial release.
  if (d.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='approval_queue'").get()) {
    ensureColumn(d, 'approval_queue', 'posted_at', 'TEXT');
    ensureColumn(d, 'approval_queue', 'posted_targets', 'TEXT');
  }
}
...
function initSchema(d) {
  d.exec(`
    ...
    -- posted_at / posted_targets are set by the auto-post worker once a
    -- video has actually been pushed somewhere downstream.
